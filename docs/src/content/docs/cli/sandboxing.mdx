...
`--dangerouslySkipPermissions` is a shortcut for `--mode dangerous` — it wins
...
| `codex` | — | Not implemented yet; selecting this harness throws. |
...
`--sandbox srt`/`e2b`/`process` only apply to a harness PrismaLens spawns as
a child process — today that's just `deepagents` (driven over ACP).
`claude-code` runs in-process via the Agent SDK, so there's no child for a
sandbox provider to wrap.

Ask for an enforced boundary (`srt` or `e2b`) with `claude-code` and the
command fails fast, before doing anything, rather than silently claiming an
enforcement it can't deliver. `--sandbox auto` or `--sandbox process` with
`claude-code` just runs with no boundary at all — no error, because nothing
dishonest is being claimed either.
...
- Leave `--mode` at the default `read-only` unless you have a specific
  reason to loosen it. `--dangerouslySkipPermissions` means exactly what it
  says, on any harness.
...
- [Harnesses](/cli/harnesses/) — what each harness needs installed.
