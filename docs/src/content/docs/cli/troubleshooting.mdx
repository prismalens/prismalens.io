...
| --- | --- | --- |
| `deepagents` (default) | `deepagents-acp` | `npm install -g deepagents-acp` |
...
environment: `ANTHROPIC_API_KEY`, `OPENAI_API_KEY`, `OLLAMA_API_KEY`,
`GOOGLE_API_KEY`, `GROQ_API_KEY`, or `CUSTOM_LLM_API_KEY`. For `claude-code` it
...
```
Sandbox 'auto' degraded to the process (cooperative, not an OS boundary):
