...
`doctor` passes this check as soon as it finds **any** supported LLM credential
env var set — the canonical list is in
[Providers → Checking your setup](/cli/providers/#checking-your-setup). For
...
**Symptom:** on the default `agent.sandbox: auto` you see a warning that the
sandbox degraded to the `process` floor (`srt egress bridge unhealthy — WSL
mirrored networking?`); or, with `--sandbox srt` forced, every tool call that
...
[Sandboxing → the WSL caveat](/cli/sandboxing/#the-wsl-caveat-mirrored-networking-breaks-srt).
