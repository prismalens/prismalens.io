...
[Providers](/cli/api-keys/) for the credential list.
:::
...
agent:
  # Which agent harness to rent: deepagents | claude-code
  # (codex is a reserved value — planned, not yet available)
  default: claude-code
  ...
  # Maximum number of turns (tool calls) the harness can make before being halted.
  # Unset means the harness uses its own default.
  # max_turns: 40

# Tier-1 synthesis provider and model. Used to synthesize the final report.
# synth:
#   provider: openai        # overrides auto-selection
#   model: gpt-5-mini       # omit to let provider pick default
#   base_url: https://...   # required for 'custom' provider
...
A service catalog keyed by service name (ADR-0015). When an alert fires, its
...
Per-harness **native passthrough** (ADR-0017), keyed by harness id. Each entry
...
| --- | --- |
| `deepagents` | `native.args` is appended **verbatim** to the `deepagents-acp` CLI invocation. `native.shellAllowList` and `native.sandbox` are **rejected** with an error — the published `deepagents-acp` binary has no such flags; use the [`--sandbox`](/cli/sandboxing/) port for real isolation instead. |
...
The permission posture (`agent.permissions.mode`) and the isolation boundary
...
Every run persists to `workspace.base_dir` (`~/.prismalens` by default): a
...
```bash
# Drop old per-run workspace dirs (keeps the record store)
find ~/.prismalens/runs -maxdepth 1 -mtime +30 -type d -exec rm -rf {} +

# Full reset — removes all runs, events, and reports
rm -rf ~/.prismalens
```

Point `workspace.base_dir` at a volume with room to spare if you run `pl listen`
...
- [Providers & harnesses](/cli/api-keys/) for what each `agent.default` value needs installed
