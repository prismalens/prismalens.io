...
| --- | --- | --- | --- | --- |
| `deepagents` (default) | `deepagents-acp` | `npm install -g deepagents-acp` | Cooperative — the harness auto-approves its own permission prompts; no OS-level enforcement yet | Available |
...
agent:
  ...
  model: openai:gpt-oss:120b     # provider-prefixed in config; --model takes a bare id
```
...
- **some** LLM credential is present — any of `ANTHROPIC_API_KEY`, `OPENAI_API_KEY`, `OLLAMA_API_KEY`, `GOOGLE_API_KEY`, `GROQ_API_KEY`, `CUSTOM_LLM_API_KEY`, or a signed-in Claude Code session,
...
- Run your first investigation: [Quick Start](/getting-started/quick-start/)
