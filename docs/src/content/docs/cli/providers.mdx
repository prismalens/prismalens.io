...
That credential check is intentionally broad — it's a smoke test, not a guarantee your chosen harness will actually work. Only `OLLAMA_API_KEY`/`OPENAI_API_KEY` (for `deepagents` and the report-synthesis step) and `ANTHROPIC_API_KEY`/Claude Code login (for `claude-code`) are ever actually read during a run. Setting `GOOGLE_API_KEY`, for example, satisfies `doctor` but won't make anything work — if a run fails after `doctor` passed, [Troubleshooting](/cli/troubleshooting/#no-llm-credential-found) breaks down which credential each path actually needs.
