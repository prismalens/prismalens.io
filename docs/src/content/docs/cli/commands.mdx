...
| `--harness <name>` | enum: `deepagents` \| `claude-code` \| `codex` | `agent.default` in config (`claude-code`) | Tier-2 harness backend. `codex` is a reserved slot — selecting it throws (not implemented). |
