...
[Troubleshooting](/cli/troubleshooting/#common-investigate-failures) walks each
...
`--json`, `--output`, and [`pl report`](#report) all emit the same
...
| `fidelity` | object \| null | The honest run-fidelity record — `{ harness, mode, fidelity, mechanism, sandbox? }` — of the boundary that actually applied. See [Sandboxing → the fidelity report](/cli/sandboxing/#requested-vs-actual-the-fidelity-report). |
...
non-zero **iff a hard check fails**.
...
| LLM credential | Hard | The resolved synthesis provider and credential source (env vs `_FILE`). Live-pings the model (~1 token, creating network egress to the provider) unless `--no-ping` is set. A broken credential or missing file causes a hard, red failure naming the source. The canonical list of credentials lives in [Providers & API keys](/cli/api-keys/#checking-your-setup). |
...
- [Providers](/cli/api-keys/) — set up a model provider and harness binary.
